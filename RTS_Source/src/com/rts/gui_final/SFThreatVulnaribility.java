 ...
 * @author rasanvin
 */
public class SFThreatVulnaribility extends javax.swing.JFrame {

    SFID sfid;
    
    /** Creates new form ThreatVulnaribility */
    public SFThreatVulnaribility() {
        initComponents();
        loadTable(jTableThreatVulRegister);
        System.out.println("--->"+getThreatTypeList());
    }

     public SFThreatVulnaribility(SFID sfid) 
     {
         this.sfid= sfid;
        initComponents();
        loadTable(jTableThreatVulRegister);
...
//jComboBoxThreatType.addActionListener(new java.awt.event.ActionListener() {
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        for(int i=0;i<jTableAddThreatVul.getRowCount();i++){
            for(int j=0;j<jTableAddThreatVul.getColumnCount();j++){
                jTableAddThreatVul.setValueAt(null, i, j);
            }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        // TODO add your handling code here:
        int res=-1;
        List<SFAssetsThreatsVuln> asv = new ArrayList<SFAssetsThreatsVuln>();
        for(int i=0;i<jTableAddThreatVul.getRowCount();i++){
            res=-1;
         ...
         res=MySQLDAOFactory.getDAOFactory().getSFAssetsThreatsVulnDao().insetSFAssetsThreatsVuln(asv.get(i));
        }
        if(res!=-1){
    ...
    private void jTableAddThreatVulMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTableAddThreatVulMouseClicked
        // TODO add your handling code here:
        System.out.append("Mouse clicked....");
        if(jTableAddThreatVul.getSelectedColumn()==1) {
            ...
            List<SF_Assets> asset=MySQLDAOFactory.getDAOFactory().getSF_AssetsDao().fetchSF_AssetsListFromDb("from SF_Assets where assetsName='"+aName+"'");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        for(int i=0;i<jTableThreatVulRegister.getRowCount();i++){
            if((Boolean)jTableThreatVulRegister.getValueAt(i, 0)==true){
                ...
                if(jTableThreatVulRegister.getValueAt(i, 4)==null||jTableThreatVulRegister.getValueAt(i, 5)==null
                    ...
                    int res = MySQLDAOFactory.getDAOFactory().getSFAssetsThreatsVulnDao().insetSFAssetsThreatsVuln(asv);
                    if(res!=-1){
        ...
        loadTable(jTableThreatVulRegister);
        for(int i=0;i<jTableThreatVulRegister.getRowCount();i++){ jTableThreatVulRegister.setValueAt(false, i, 0);}
        }
...
//                int rowNum[], colNum[];
        int startIndex = -1;
        SFAssetsThreatsVulnDao atvDAO = MySQLDAOFactory.getDAOFactory().getSFAssetsThreatsVulnDao();
        ...
        for (int i = (selectColumnVal.length - 1); i >= 0; i--) {
            if (selectColumnVal[i] == true) {
                ...
                SFAssetsThreatsVuln atv = new SFAssetsThreatsVuln();
                System.out.println("jTableThreatVulRegister.getValueAt(i, 10)==:" + jTableThreatVulRegister.getValueAt(i, 10));
                if (jTableThreatVulRegister.getValueAt(i, 10)==null) {
                    ...
                    model.removeRow(i);
                    System.out.println("row"+i+"removed successfully");
                    continue;
                } else {
                    atv = readTableRow(jTableThreatVulRegister,i);
                    int row = atvDAO.deleteSFAssetsThreatsVuln(atv);
                    if (row == 0) {
...
//         String[] threatsList=MySQLDAOFactory.getDAOFactory().getThreatsDao().fetchThreatsCol("Select threatName from SFThreats where threatType='"+jTableThreatVulRegister.getModel().getValueAt(row, 4)+"'");
...
//         String[] threatsList=MySQLDAOFactory.getDAOFactory().getThreatsDao().fetchThreatsCol("Select threatName from SFThreats where threatType='"+jComboBoxThreatType.getSelectedItem()+"'");
//         jComboBoxThreatList.setModel(new DefaultComboBoxModel(threatsList));
...
//        }
    ...
    private void jComboBoxAddThreatListFocusGained(java.awt.event.FocusEvent evt){
        int row = jTableAddThreatVul.getSelectedRow();
        if(row!=-1){
        String[] threatsList=MySQLDAOFactory.getDAOFactory().getSFThreatsDao().fetchSFThreatsCol("Select threatName from SFThreats where threatType='"+jTableAddThreatVul.getModel().getValueAt(row, 2)+"'");
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new SFThreatVulnaribility().setVisible(true);
            }
        });
    }
...
//*****************************************************My methods****************
private SFAssetsThreatsVuln readTableRow(JTable table,int row){
    int i=0;
    ...
    asv.getVulnerabilities().setVulnarabilityName((String)table.getValueAt(row, 6-i));
        ...
        if(asv.getThreats().getThreatName()!=null) {
            List<SFThreats> tList=new ArrayList<SFThreats>();
        tList=MySQLDAOFactory.getDAOFactory().getSFThreatsDao().fetchSFThreats("from SFThreats where threatName='"+asv.getThreats().getThreatName()+"'");
        asv.getThreats().setThreatId(tList.get(0).getThreatId());
        }
        if(asv.getVulnerabilities().getVulnarabilityName()!=null) {
            List<SFVulnarabilities> vList=new ArrayList<SFVulnarabilities>();
        vList=MySQLDAOFactory.getDAOFactory().getSFVulnDao().fetchSFVuln("from SFVulnarabilities where vulnarabilityName='"+asv.getVulnerabilities().getVulnarabilityName()+"'");
    ...
    if(table==jTableAddThreatVul) {
        ...
        aList=MySQLDAOFactory.getDAOFactory().getSF_AssetsDao().fetchSF_AssetsListFromDb("from SF_Assets where assetsName='"+asv.getAssets().getAssetsName()+"'");
...
private void loadTable(JTable table) {
        ...
        List<SFAssetsThreatsVuln> assetThreatsVulnList = new ArrayList<SFAssetsThreatsVuln>();
        List<SF_Assets> assets = new ArrayList<SF_Assets>();
        int index=0;
        int foundInASV=0;
        assets = MySQLDAOFactory.getDAOFactory().getSF_AssetsDao().fetchSF_AssetsListFromDb("from SF_Assets where SFId.SFId="+sfid.getSFId());
// Use This    assets = MySQLDAOFactory.getDAOFactory().getAssetsDao().fetchAssetsListFromDb("from Assets where projectId="+projectid);
       // String query="from AssetsThreatsVuln a where a.getAssets().getAssetId()="+assets.get(1).getAssetId();

        assetThreatsVulnList=MySQLDAOFactory.getDAOFactory().getSFAssetsThreatsVulnDao().fetchSFAssetsThreatsVuln("from SFAssetsThreatsVuln where assets.SFId.SFId="+sfid.getSFId());
